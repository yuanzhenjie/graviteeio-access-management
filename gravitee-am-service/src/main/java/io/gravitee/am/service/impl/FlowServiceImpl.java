...
public class FlowServiceImpl implements FlowService {
    ...
    private Flow buildFlow(Type type, ReferenceType referenceType, String referenceId) {
        ...
        if (Type.ROOT.equals(type)) {
            ...
            all.setName(type.name());
            all.setPost(new ArrayList<>());
        }
        all.setPre(new ArrayList<>());
        all.setType(type);
        all.setReferenceType(referenceType);
        all.setReferenceId(referenceId);
        return all;
    }
    ...
    public Single<Flow> create(ReferenceType referenceType, String referenceId, Flow flow, User principal) {
        ...
        if (Type.ROOT.equals(flow.getType())) {
            flow.setPost(null);
        }
